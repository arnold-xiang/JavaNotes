...
* **栈**：**stack**---它是运算受限的**线性表**，其限制是仅允许在标的一端进行插入和删除操作，不允许在其他任何位置进行添加、查找、删除等操作。
...
*  查找元素快：通过索引，可以快速访问指定位置的元素
   ...
   *  原因：数组的地址是连续的，通过数组的首地址可以找到数组，通过数组的索引可以快速查找某一个元素

   ![](img/数组查询快.png)

*  增删元素慢
  
  *  原因：长度是固定的，增加/删除一个元素，必须创建一个新数组，把原数组的数据复制过来
...
* 我们常说的链表结构有单向链表（只有一条链，不能保证元素的顺序）与双向链表（两条链，有一条专门记录数据）。
...
* 查找元素慢：地址是不连续的，想查找某个元素，每次都需要通过连接的节点，依次向后查找指定元素
...
简单的理解，就是一种类似于我们生活中树的结构，只不过**每个结点上都最多只能有两个子结点**。
...
- 它是一个元素**存取有序**的集合。例如，存元素的顺序是11、22、33。那么集合中，元素的存储就是按照11、22、33的顺序完成的）。

- 它是一个**带有索引**的集合，通过索引就可以精确的操作集合中的元素（与数组的索引是一个道理）。

- 集合中**可以有重复**的元素，通过元素的equals方法，来比较是否为重复的元素。
...
- `java.util.LinkedList`集合数据存储的底层结构是链表结构----增删快，查询慢
...
- 没有索引
- 元素不出现重复---调用add方法的时候hashCode和equals方法
- 元素无序
...
- 哈希值：一个十进制的整数，由系统随机给出（对象的地址值，是一个逻辑地址，不是真实的物理地址）
- `Object`类的方法`hashCode()`方法获取对象哈希值
- `hashCode()`方法源码

```java
public native int hashCode();//native表示调用本地操作系统的方法
```

什么是哈希表呢？

在**JDK1.8**之前，哈希表底层采用`数组+链表`实现，即使用链表处理冲突，同一hash值的链表都存储在一个链表里。但是当位于一个桶中的元素较多，即hash值相等的元素较多时，通过key值依次查找的效率较低。而JDK1.8中，哈希表存储采用数组+链表+红黑树实现，当链表长度超过阈值（8）时，将链表转换为红黑树，这样大大减少了查找时间。

简单的来说，哈希表是由`数组+链表+红黑树`（JDK1.8增加了红黑树部分）实现的，如下图所示。![](img\哈希表.png)
...
- `public static <T> void sort(List<T> list)`:将集合中元素按照默认规则排序。
  - 如果使用自定义类型，则必须实现`Comparable`接口，重写`compareTo`方法：定义排序的规则
  - 规则：`this - 参数` ---> 升序
...
- 自己（this）和别人（参数）比较，自己需要实现实现`Comparable`接口，重写`compareTo`方法：定义排序的规则
