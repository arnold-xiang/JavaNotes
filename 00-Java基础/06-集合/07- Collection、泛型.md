...
![](F:\黑马 JavaEE 57期 新\讲义+笔记+资料\01-Java基础\06.集合\13.【Collection、泛型】-笔记\resource\01_集合框架介绍.bmp)
...
在程序开发中，经常需要**遍历**集合中的所有元素。针对这种需求，JDK专门提供了一个接口`java.util.Iterator`。`Iterator`接口也是Java集合中的一员，但它与`Collection`、`Map`接口有所不同，`Collection`接口与`Map`接口主要用于存储元素，而`Iterator`主要用于迭代访问（即遍历）`Collection`中的元素，因此`Iterator`对象也被称为迭代器。
...
- 使用集合的`iterator()`方法获取迭代器的实现类对象，使用`Iterator`接口接收（多态）
- 使用`Iterator`接口中的`hasNext`方法判断还有没有下一个元素
- 使用`Iterator`接口中的`next`方法取出集合中的下一个元素
...
![](F:\黑马 JavaEE 57期 新\讲义+笔记+资料\01-Java基础\06.集合\13.【Collection、泛型】-笔记\resource\02_迭代器的实现原理(1).bmp)
...
~~~java
package cn.arnold.day05.demo02;

public class GenericInterfaceMain {
    public static void main(String[] args) {
        //接口实现类确定泛型
        GenericInterfaceImpl01 gif = new GenericInterfaceImpl01();
        gif.method("Arnold");
        //创建接口实现类对象时确定泛型
        GenericInterfaceImpl02<Double> gifE =new GenericInterfaceImpl02<>();
        gifE.method(5.0);
    }
}
~~~
...
public class GeneriTongPeiFuMain {
    public static void main(String[] args) {
    ...
    //不知道集合中的数据类型是什么类型，?代表可以接收任意类型
    public static void getElements(ArrayList<?> arrayList) {
        Iterator coll = arrayList.iterator();
        while (coll.hasNext()) {
            Object o = coll.next();
